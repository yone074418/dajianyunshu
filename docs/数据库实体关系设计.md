...
| Q-11 | 14项系统评价计算口径 | PCR-008/EVAL-POL-001已消除端点冲突并冻结周期、缺项、0次和舍入 | scores | 已关闭 |
| Q-12 | 12项教师评价具体名称 | 使用论文表4.2名称 | teacher_reviews | 课程负责人 |
| Q-13 | 12项教师评价评分口径 | EVAL-POL-001确定5分最好、1分最低 | teacher_reviews | 已关闭 |
...
本节是PDEC-006批准后的候选基线追加设计，不编写SQL或物理字段；原第11天历史验收结论不回写。

### 25.1 新增逻辑实体

| 实体 | 中文名 | 目的 | 关系 | 局部状态 | 首版权限 |
|---|---|---|---|---|---|
| learning_activities | 异步学习活动 | 保存提问、讨论主题及师生回复的原始取数证据 | profiles 1:N；主题1:N回复；可关联cases/courses | draft、submitted；回复仅submitted | 作者写本人草稿/提交；参与者新增回复；授权教师读/回复；禁止改他人原文 |
| reports | 学习报告 | 保存预习报告与实验报告原文及提交证据 | profiles 1:N；可关联cases、attempts | draft、submitted | 学生写本人草稿/提交；授权教师只读已提交；禁止代改 |

`learning_progress`继续承载阅读进度和笔记，不重复创建笔记实体。上述局部状态与15个实验生命周期状态完全分离。

### 25.2 RLS、一致性与评价

- 学生按本人身份隔离；教师每次读取均校验`teacher_student_scopes`，无授权返回统一拒绝且不泄露记录存在性。
- 保存与提交必须幂等；重复请求不得增加活动、回复、报告、日志或评价计数。
- 刷新、断网和重登恢复最近确认版本；未确认本地内容不得标记为已提交。
- `operation_logs`追加保存、提交、拒绝与重试审计；日志失败待重试但不改变活动结论。
- `learning_activities`为C1—C5、D3提供原始事件，`reports`为A7、A8、D4、D6提供证据；缺项保持null，不得补0。

### 25.3 数据边界结论

本补充使GAP-001、BLOCK-005、ACT-007具备数据对象、RLS、恢复和幂等证据。具体字段、索引和幂等键组成在第23天迁移设计中确定，不由实体名称反向创造新业务状态。
