...
- Day43-47：当前 main 已包含 Day43-46，本次同步合并 Day47 并接入 Day48 提示框架 ✅
- G3 验收（Day42）：通过 ✅
...
- `TaskIntroductionPage.test.tsx` — 覆盖任务介绍页提示面板渲染和点击查看提示

## 9. 页面接入

- `TaskIntroductionPage.tsx` 已接入 `CurrentStepHintPanel`
- 当前步骤：`stage_1_task_intro`
- 步骤名称：运输任务及货物介绍
- 用户在任务介绍页查看提示时，会写入 `hint_viewed` 日志并增加提示计数
...
| `npm run test:run`     | ✅ 当前主线 29 个测试文件、300 个测试通过 |
| `npm run test:e2e`     | ✅ 10 通过                                |
| `npm run build`        | ✅ 当前主线验证通过                       |
| `git diff --check`     | ✅ 无错误                                 |
